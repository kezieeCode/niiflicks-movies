...
class _PlayState extends State<Play> {
  ...
  String getTime() {
    String min, secs;
    min = position.inMinutes > 9
        ? '${position.inMinutes}'
        : '0${position?.inMinutes}';
    secs = (position.inSeconds - 60 * int.parse(min)) > 9
        ? '${position.inSeconds - 60 * int.parse(min)}'
        : '0${position.inSeconds - 60 * int.parse(min)}';
    return '$min:$secs';
  }

  void nextSong(bool value) {
    if (value) player.setNextPlayed(true);
    player.stop();
    widget.index != widget.songs.length - 1
        ? setState(() {
            ...
            isAlbum
                ? AlbumSongs.indexSelected = widget.index
                : Songs.indexSelected = widget.index;
  ...
  void prevSong() {
    ...
    if (widget.index != 0)
      setState(() {
        ...
        widget.currentSong = widget.songs[widget.index];
        isAlbum
            ? albumState.setState(() => AlbumSongs.indexSelected = widget.index)
            : stateSong.setState(() => Songs.indexSelected = widget.index);
        player.currentSong = widget.currentSong;
      });
    else
      setState(() {
  ...
  void toggle() {
    ...
    if (tabState.mounted) setState(() => playIcon);
    setState(() {
      play;
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    positionSubscription =
        player.audioPlayer.onAudioPositionChanged.listen((p) {
        ...
        child: Column(children: <Widget>[
            ...
            children: <Widget>[
              Container(
                ...
                child: BackdropFilter(
                  ...
                  child: new Container(
                    decoration:
                        new BoxDecoration(color: Colors.white.withOpacity(0.4)),
                  ),
                ),
              ),
              Column(
          ...
          Padding(
            padding: EdgeInsets.only(
                left: 10.0, right: 10.0, top: height * 0.05, bottom: 5),
            child: Text(
          ...
          Slider(
            ...
            min: 0.0,
            max: widget?.currentSong?.time?.floorToDouble(),
          ),
          Padding(
            ...
            children: <Widget>[
              InkWell(
                  child: Icon(Icons.skip_previous,
                      size: size.width * 0.15, color: kDefaultIControlsColor),
                  onTap: () => prevSong()),
              InkWell(
                  child: Icon(
                    playIcon,
                    size: size.width * 0.22,
                    color: kDefaultIControlsColor,
                  ),
                  onTap: () => toggle()),
              InkWell(
                  child: Icon(
