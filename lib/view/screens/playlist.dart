...
import 'package:niiflicks/view/widget/album-grid-item.dart';
...
void addSongs(List<Song> songList) {
          ...
          onTap: () {
            if (isNameAvailable(songList, playlist[index])) {
              ...
              showMessage(scaffoldState.currentState,
                  '${songList[0].title} ${songList.length - 1 > 0 ? '+ ${songList.length - 1}' : ''} ${MyLocalizations.of(scaffoldState.currentContext).added}');
            }
  ...
  showDialog(
      ...
      builder: (_) => SimpleDialog(
            title: Text(
                '${MyLocalizations.of(scaffoldState.currentContext).select}'),
            children: <Widget>[
              Column(
                children: <Widget>[
                  ListTile(
                    title: Text(
                        '${MyLocalizations.of(scaffoldState.currentContext).create} ${MyLocalizations.of(scaffoldState.currentContext).playlist}'),
                    onTap: () {
                      Navigator.pop(scaffoldState.currentContext);
                      createPlaylist(scaffoldState.currentState,
                          songs: songList);
                    },
...
void createPlaylist(ScaffoldState scaffold, {List<Song> songs}) {
  ...
  showDialog(
      ...
      builder: (_) => SimpleDialog(
              title: Text(
                  '${MyLocalizations.of(scaffold.context).create} ${MyLocalizations.of(scaffold.context).playlist}'),
              children: <Widget>[
                TextField(
                  ...
                  decoration: InputDecoration(
                      hintText:
                          '  ${MyLocalizations.of(scaffold.context).enter}',
                ...
                Padding(
                  ...
                  child: Row(
                    ...
                    children: <Widget>[
                      RaisedButton(
                        child:
                            Text(MyLocalizations.of(scaffold.context).create),
                        onPressed: () {
                          if (controller.text.isEmpty) //check for blank name
                            showMessage(scaffold,
                                '${MyLocalizations.of(scaffold.context).invalidName}');
                          else {
                            bool available = true;
                            for (int i = 0; i < playlist.length; i++) {
                              ...
                              if (controller.text == playlist[i].name) {
                                available = false;
                                showMessage(scaffold,
                                    '${controller.text} ${MyLocalizations.of(scaffold.context).already}');
                                break;
                              }
                            }
                            if (available) //adding song if name is valid
                            {
                              if (songs == null) //blank playlist
                                playlist.add(
                                    Album(controller.text, kPlaceholderPath));
                              else {
                                playlist.add(
                                    Album(controller.text, songs[0].albumArt));
                                playlist.last.addAlbum(songs);
                                showMessage(scaffold,
                                    '${songs[0].title} ${songs.length - 1 > 0 ? '+ ${songs.length - 1}' : ''} ${MyLocalizations.of(scaffold.context).added}');
                              }
                              editFile();
                      ...
                      RaisedButton(
                        onPressed: () => Navigator.pop(scaffold.context),
                        child:
                            Text(MyLocalizations.of(scaffold.context).cancel),
...
class _PlaylistState extends State<Playlist> {
  ...
  void deletePlaylist(int index) {
    showDialog(
        ...
        builder: (_) => SimpleDialog(
              title: Text(
                  '${MyLocalizations.of(context).delete} ${playlist[index].name}'),
              children: <Widget>[
                Row(
                  mainAxisAlignment: MainAxisAlignment.spaceAround,
                  children: <Widget>[
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: !isSearch
            ? Text(
                MyLocalizations.of(context).playlist,
                style: TextStyle(
                  color: Colors.black,
                  fontSize: 22.0,
                  letterSpacing: 1.5,
        ...
        actions: <Widget>[
          ...
          isLongPressed
              ? IconButton(
                  ...
                  onPressed: () => deletePlaylist(selectedIndex))
              : IconButton(
                  icon: Icon(Icons.add_circle),
                  onPressed: () => createPlaylist(myScaff.currentState))
        ],
      ),
      body: Container(
        child: playlist.isEmpty
            ? Container(
                ...
                child: Column(
                  ...
                  children: <Widget>[
                    Text(
                      MyLocalizations.of(context).noPlaylist,
                      style: TextStyle(
                          fontSize: 25.0, letterSpacing: 1.0, wordSpacing: 3.0),
                      textAlign: TextAlign.center,
                    ),
                    Image(
                ...
                itemBuilder: (BuildContext context, int index) {
                  return InkWell(
                    child: Padding(
                      ...
                      child: GridTile(
                        child: AlbumItem(
                          image: playlist[index].album.isNotEmpty
                              ? playlist[index].album[0].albumArt
                              : null,
