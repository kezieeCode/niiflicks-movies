...
class AppleMusicStore {
  ...
  static const JWT_KEY = 'frXkBrLYqJzL38J5BD8alATx9ZyAZghCrsukIG9wUh9IkyceSOa';
  // String url = Uri.parse('') as String;
  Future<dynamic> _fetchJSON(String uri) async {
    final response = await http
        .get(Uri.parse(uri), headers: {'Authorization': "Bearer $JWT_KEY"});
    print(response.body);
  ...
  Future<Search> search(String query) async {
    ...
    if (artistJSON != null) {
      artists.addAll((artistJSON).map((a) => Artist.fromJson(a, null, null)));
    }
