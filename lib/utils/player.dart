...
class MyPlayer {
  ...
  void nextSong(bool value) {
    ...
    if (state.mounted) state.setState(() => state.song(currentSong));
  }
  ...
  void toggle() {
    ...
    if (tabState != null && tabState.mounted) tabState.setState(() => playIcon);
  ...
  void generateNotification() async {
    ...
    //         isOngoing: true, vibratePattern: AndroidVibratePatterns.NONE),
    //     actions: [
  ...
  void onReplyClick(String payload) {
    if (payload == 'previousSong')
      ...
      toggle();
    else if (payload == 'nextSong') nextSong(true);
  }
}
